...
public class FilmeDAO extends DAO {
	...
	public boolean insert(Filme filme) {
		...
		try {
			...
			String sql = "INSERT INTO filme (ID_Filme, Generos, Atores, Titulo, Classificacao_Indicativa, Descricao, Data_Lancamento)"
					+ "VALUES (" + filme.getID_Filme() + ", '" + filme.getGeneros() + "', '"
					+ filme.getAtores() + "', '" + filme.getTitulo() + "', '" + filme.getClassificacao_Indicativa()
					+ "', '"
					+ filme.getDescricao() + "', '" + filme.getData_Lancamento() + ")";
	...
	public Filme get(int ID_Filme) {
		...
		try {
			...
			String sql = "SELECT * FROM filme WHERE ID=" + filme;
			System.out.println(sql);
			...
			if (rs.next()) {
				...
				filme = new Filme(rs.getInt("ID_Filme"), generos, rs.getString("Atores"),
						rs.getString("Titulo"), rs.getString("Classificacao_Indicativa"), rs.getString("Descricao"),
	...
	public List<Filme> getOrderByAtores() {
		return get("Atores");
	}
	...
	private List<Filme> get(String orderBy) {
		...
		try {
			...
			while (rs.next()) {
				...
				Filme a = new Filme(rs.getInt("ID_Filme"), generos, rs.getString("Atores"),
						rs.getString("Titulo"), rs.getString("Classificacao_Indicativa"), rs.getString("Descricao"),
						rs.getString("Data_Lancamento"));
				filmes.add(a);
			}
	...
	public boolean update(Filme filme) {
		...
		try {
			...
			String sql = "UPDATE filme SET ID_Filme = '" + filme.getID_Filme() + "', Generos = '"
					+ filme.getGeneros() + "', Atores = '" + filme.getAtores() + "', Titulo = '" + filme.getTitulo()
